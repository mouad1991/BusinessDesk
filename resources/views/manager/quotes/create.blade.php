...
@section('breadcrumb')
    <a href="{{ route('manager.quotes.index') }}">Devis</a> / Nouveau
@endsection

@section('content')
<form method="POST" action="{{ route('manager.quotes.store') }}" id="doc-form">
    @csrf
        ...
        <div class="card">
            ...
            <div class="card-body">
                <div class="form-row">
                    ...
                    <div class="form-group">
                        <label>Validité (jours) <span class="required">*</span></label>
                        <input type="number" name="validity_days" value="{{ old('validity_days', 30) }}" min="1" max="365" required>
                    </div>
                </div>
                <div class="form-group">
                    <label>Mode de paiement</label>
                    <input type="text" name="payment_mode" value="{{ old('payment_mode', '50% à la commande / 50% à la réception') }}" placeholder="Ex : 50% à la commande / 50% à la réception">
                </div>
                <div class="form-group">
                    <label>Objet <span class="required">*</span></label>
                    <input type="text" name="subject" value="{{ old('subject') }}" class="{{ $errors->has('subject') ? 'is-invalid' : '' }}" required placeholder="Objet du devis">
    ...
    <div class="card mt-4">
        ...
        <div class="card-body">
            <div class="form-group">
                <label>Note</label>
                <textarea name="notes" rows="3" placeholder="Ex : Conditions particulières, mention spécifique au client, remarque...">{{ old('notes') }}</textarea>
            </div>
        </div>
    </div>

    <div class="form-actions mt-4">
        <a href="{{ route('manager.quotes.index') }}" class="btn-secondary">Annuler</a>
